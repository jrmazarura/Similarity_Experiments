...
data = pd.read_csv('pan12-conversations.csv')

#selected desired columns
conversation=data.filter(['updated_content', 'label'])
conversation.rename(columns = {'updated_content': 'text'}, inplace = True)

#clean data
conversation['tokenized']  = conversation['text'].apply(lambda x: stem_words(clean(str(x))))

#split data
pred_indicator=(conversation['label'] == 1)
pred1 = conversation[pred_indicator]

non_pred_indicator = (conversation['label'] == 0)
non_pred1 = conversation[non_pred_indicator]


corpus_stats(pred1,'Predatory Conversations')
corpus_stats(non_pred1,'Non-Predatory Conversations')

#cut documents that are too long and remove documents that are too short
min_length=1
max_length=100

pred=remove_docs(pred1,min_length,max_length)
non_pred=remove_docs(non_pred1,min_length,max_length)

corpus_stats(pred,'Predatory Conversations')
corpus_stats(non_pred,'Non-Predatory Conversations')

#split datasets: testing/training
pred_train, pred_test= train_test_split(pred, test_size=0.2, random_state=1)

sample_of_nonpred=non_pred.sample(n=1787,random_state=1,axis=0)
non_pred_train, non_pred_test= train_test_split(sample_of_nonpred, test_size=0.2, random_state=1)

#concatenate: create train and test data
train = pd.concat((pred_train,non_pred_train))
train.reset_index(drop=True, inplace=True)

test = pd.concat((pred_test,non_pred_test))
...
save_obj(dct, 'GPMDICT-pan12_train')
corpus = [dct.doc2bow(line) for line in train_df['tokenized']]  # bowCorpus
save_obj(train_df, 'GPM_training_PAN2012')
...
#testing
df_test=test
save_obj(df_test, 'GPM_test_PAN2012')
...
dtd_test_pred=doc_topic_dist(pred_test['tokenized'])
dtd_test_non_pred=doc_topic_dist(non_pred_test['tokenized'])

# Reference corpus: Predatory
sim_ref=dtd_topic[1]
sim_query=dtd_test_pred

js_check1 = jsd_mat(sim_ref, sim_query)
js_ch1_mean = np.mean(js_check1, axis=0)

ref = jsd_mat(sim_ref, sim_ref)
ref = ref[~np.eye(ref.shape[0], dtype=bool)].reshape(ref.shape[0], -1) 
...
# Query corpus: Non_predatory
disim_query=dtd_test_non_pred
...
save_obj(prob_disimGPM, 'probdisim_w2v-application_PAN2012')
save_obj(prob_simGPM, 'probsim_w2v-application_PAN2012')

js_plots([js_check1,js_check2],['Predatory (ref) vs Predatory (query)','Predatory (ref) vs Non-predatory (query)'],color=['green','red'])

############################################################################################################################
#WORD2VEC
print('Running word2vec...')
      
df = load_obj('GPM_training_PAN2012')
...
dictionary = load_obj('GPMDICT-pan12_train')

bow_corpus = [dictionary.doc2bow(document) for document in df['tokenized']]  # generate a bow corpus
similarity_matrix = SparseTermSimilarityMatrix(termsim_index, dictionary)

df_test_dominant_topic=load_obj('GPM_test_PAN2012')

test1=df_test_dominant_topic['label']==1 
test0=df_test_dominant_topic['label']==0 
...
sns.distplot(scs_topic1, bins=50, color='red', label='Predatory (Testing vs Training)')
sns.distplot(scs_topic1vstopic0, bins=50, color='green', label='Predatory (Testing) vs Non-Predatory (Training)')
...
docsim_index = SoftCosineSimilarity(top1_ref, similarity_matrix)  # SCM with ref set for later use
...
save_obj(prob_simW2V, 'probsim_w2v-application_PAN2012')
save_obj(prob_disimW2V, 'probdisim_w2v-application_PAN2012')

plt.rcParams['xtick.labelsize'] = 15
plt.rcParams['ytick.labelsize'] = 15
js_plots_stacked([js_check1, js_check2],[scs_topic1, scs_topic1vstopic0], ['Predatory (ref) vs Predatory (query)', 'Predatory (ref) vs Non-Predatory (query)'], ['Predatory (ref) vs Predatory (query)', 'Predatory (ref) vs Non-Predatory (query)'])

############################################################################################################################
#CLASSIFICATION METRICS
print('Calculating classification metrics...')

sim_prob = load_obj('sim_probs_applicationGPM_PAN2012')
disim_prob = load_obj('disim_probs_applicationGPM_PAN2012')
...
perc_ignoreGPM = float(len(true_irrelevantGPM)) / (len(df)) * 100
precision = float(len(true_relevant)) / float(len(false_irrelevant) + len(true_relevant))
accuracy = (len(true_relevant) + len(true_irrelevantGPM)) / len(df)
recall = float(len(true_relevant)) / float(len(false_relevant) + len(true_relevant))
...
sim_prob = load_obj('probsim_w2v-application_PAN2012')
disim_prob = load_obj('probdisim_w2v-application_PAN2012')
...
perc_ignore = float(len(true_irrelevant)) / (len(df)) * 100 
precision = float(len(true_relevant)) / float(len(false_irrelevant) + len(true_relevant))
...
[axs[k].set_xlim([0, len(labels)]) for k in range(2)]
...
plt.savefig('PHD_PAN_2012_RELINDEX.jpg', bbox_inches='tight')
plt.show()
